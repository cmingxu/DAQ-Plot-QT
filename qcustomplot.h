...
protected:
  ...
  void DAQMemcpy(QVector<DataType>& dst_vector,const QVector<DataType>& src_vector){
    if(dst_vector.size() != src_vector.size()){
      dst_vector.resize(src_vector.size());
    }
#ifdef USE_RGA
    ...
    int buf_size = src_vector.size() * 1  * sizeof(DataType);
    int ret = 0;
    double scale = sizeof(DataType) / get_bpp_from_format(RK_FORMAT_RGBA_8888);
    src_img = wrapbuffer_virtualaddr((void*)src_vector.data(), src_vector.size() / 250, 250 * scale, RK_FORMAT_RGBA_8888);
    dst_img = wrapbuffer_virtualaddr((void*)dst_vector.data(), dst_vector.size() / 250, 250 * scale, RK_FORMAT_RGBA_8888);
...
#else
    memcpy(dst_vector.data(),src_vector.data(),src_vector.size() * sizeof(DataType));
#endif
  }
